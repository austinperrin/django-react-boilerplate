...
- Use `rg` for fast repo searches (preferred over `grep`). Add `--hidden` when you need to include hidden files/folders (e.g., `.github`, `.env.backend`, and `.env.frontend`).
...
- Python linting/tooling config lives in the repo root `pyproject.toml` (ruff/black/mypy).
