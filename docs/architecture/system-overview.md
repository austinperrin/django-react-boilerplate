...
- **Backend (`services/backend`)**: Django/DRF API exposing versioned endpoints (`/api/v1/<domain>/`). Handles identity, RBAC, tenant config, and business domains (planned scope).
- **Frontend (`services/frontend`)**: React/Vite app hosting tenant-defined login portals and consuming the backend API via JWT-authenticated calls (planned scope).
...
1. User hits login portal URL → frontend loads entry point metadata and renders the login UI (planned).
2. User authenticates (username/password, SSO, etc.) → backend issues JWT with tenant + entry point context (planned).
3. Frontend stores token securely (HTTP-only cookie) and uses it for API requests.
4. Backend enforces RBAC per endpoint; responses include tenant-scoped data pulled from the tenant’s Postgres DB (planned).
