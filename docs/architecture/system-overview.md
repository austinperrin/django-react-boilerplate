...
- **Frontend (`services/frontend`)**: React/Vite app hosting tenant-defined login portals and consuming the backend API via JWT-authenticated calls.
...
1. User hits login portal URL → frontend loads entry point metadata and renders the login UI.
2. User authenticates (username/password, SSO, etc.) → backend issues JWT with tenant + entry point context.
