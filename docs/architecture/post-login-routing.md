...
- Route guards use RBAC claims from the JWT to allow or block navigation.
- Feature flags toggle UI at the module or component level.

## Backend

- JWT claims include tenant ID, entry point slug, roles, and permissions.
- Protected endpoints enforce RBAC server-side regardless of client routing.
