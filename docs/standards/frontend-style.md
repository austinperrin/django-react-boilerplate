...
- Source code lives in `services/frontend/src/` using an `apps/<area>` pattern for post-login app areas.
...
- Ensure responsiveness for desktop/tablet/mobile by default. Document any login-portal-specific constraints.

## State Management & Data

- Use React Query (or similar) for server state; avoid ad-hoc fetch calls scattered across components.
- Centralize auth/token handling inside a context provider; post-login areas read RBAC claims via hooks.
...
- Enforce accessibility lint rules (jsx-a11y). Each login portal and post-login area must meet WCAG AA baseline.
...
- Add Cypress/Playwright E2E coverage per login portal and post-login area once flows stabilize.
