...
|-----------------|-------------------------------------------|----------------------------------------------------------|
| tenant-admin    | Full control over tenant configuration    | Manage portals, configure IAM, view audit logs           |
| support         | Assist users without accessing sensitive data | View user status, reset passwords, manage tickets    |
| standard-user   | Default role for typical portal users     | View personal data, submit forms, receive notifications  |
| custom-<slug>   | Tenant-defined role                       | Determined by tenant configuration                       |

## Permission Concepts

- Permissions should describe actions (e.g., `portal.view_reports`, `identity.manage_users`).
